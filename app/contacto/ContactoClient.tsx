...
// ponytail: test key de Cloudflare como fallback; en prod entra la real por env.
...
export default function ContactoClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!tokenRef.current) {
      setServerError("Completa la verificación anti-spam.");
      return;
    }
    setStatus("loading");
    setServerError("");
        ...
        <section className="bg-[#feeaed] rounded-[20px] md:rounded-[30px] px-6 py-10 md:px-20 md:py-16">
          ...
          <form onSubmit={handleSubmit} noValidate className="grid grid-cols-1 md:grid-cols-[1.1fr_1fr] gap-x-8 gap-y-5 md:gap-y-2">
            ...
            <div className="md:col-span-3 mt-2 flex flex-col gap-3">
              ...
              {status === "success" && (
                <p className="font-satoshi text-[#232321] text-[15px] font-medium">
