...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "gestion-de-residuos" });
  return {
    title: t("metaTitle"),
    description: t("metaDesc"),
    alternates: buildAlternates(locale, "/casos-de-uso/empresa-de-gestion-de-residuos"),
  };
}

export default async function EmpresaDeGestionDeResiduos({
  params,
}: {
  ...
  const t = await getTranslations("gestion-de-residuos");
      ...
      <div className="px-4 md:px-0 pb-16 flex flex-col gap-6 max-w-7xl mx-auto">
        ...
        <section className="bg-[#feeaed] rounded-[20px] px-6 py-10 md:px-[70px] md:py-[60px] text-center">
          <p className="font-satoshi text-[22px] md:text-[32px] font-bold text-[#192437] leading-[30px] md:leading-[36px] max-w-[780px] mx-auto">
            {t("ctaText1")}<span className="text-[#e22939]">{t("ctaHighlight")}</span>{t("ctaText2")}
          </p>
        </section>
