...
const UC_ICONS = [faSoap, faDesktop, faTrashAlt, faBroadcastTower, faBug];
const UC_HREFS = [
...
export default function UseCasesCarousel() {
  const t = useTranslations("cases-carousel");

  const USE_CASES = [
    { icon: UC_ICONS[0], title: t("higieneTitle"), description: t("higieneDesc"), href: UC_HREFS[0] },
    { icon: UC_ICONS[1], title: t("tecnicoTitle"), description: t("tecnicoDesc"), href: UC_HREFS[1] },
    { icon: UC_ICONS[2], title: t("residuosTitle"), description: t("residuosDesc"), href: UC_HREFS[2] },
    { icon: UC_ICONS[3], title: t("telecomTitle"), description: t("telecomDesc"), href: UC_HREFS[3] },
    { icon: UC_ICONS[4], title: t("plagasTitle"), description: t("plagasDesc"), href: UC_HREFS[4] },
  ];
  ...
  const mobileSlideClass = mobileAnimating
    ? mobileDirection === "right" ? "animate-uc-out-left" : "animate-uc-out-right"
    : mobileDirection === "right" ? "animate-uc-in-right" : "animate-uc-in-left";
  ...
  const cardContent = (c: (typeof USE_CASES)[number]) => (
    <>
      <div className="w-[60px] h-[60px] md:w-[80px] md:h-[80px] rounded-full bg-white flex items-center justify-center">
        <FontAwesomeIcon icon={c.icon} className="text-[#e42233] group-hover:text-[#c01f2d] transition-colors duration-300" style={{ fontSize: "2.5rem" }} />
      </div>
    ...
    <>
      ...
      >
        ...
        <div className={`px-10 ${mobileSlideClass}`} key={mobileAnimating ? "exit" : mobileCurrent}>
          <Link href={USE_CASES[mobileCurrent].href} className="bg-[#feeaed] rounded-[20px] p-6 flex flex-col items-center text-center">
